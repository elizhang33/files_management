...
logging.basicConfig(level=logging.INFO,
                    ...
                    filename='files_report.log',  # Name of the log file
...
def main():
    try:
        ...
        time_readable = strftime('%Y-%m-%d %H:%M:%S', localtime(start_time))
        print("start_time", time_readable)
        ...
        max_brand_length = max(len(brand) for (brand, _), _ in image_counts.items()) + 2
        max_floorset_length = max(len(floorset) for (_, floorset), _ in image_counts.items()) + 2
        max_count_length = max(len(str(count) + ' images') for _, count in image_counts.items()) + 2
        #max_full_set_count_length = max(len(str(full_set_count) + ' full sets') for full_set_count in full_set_counts.values()) + 2
        #the full_set_count might be empty if there's no full set found in the count_images_and_full_sets() function. 
        if full_set_counts:
            max_full_set_count_length = max(len(str(full_set_count) + ' full sets') for full_set_count in full_set_counts.values()) + 2
        else:
            max_full_set_count_length = 5
        
        
        # Build the message string
        message_lines = []
        message_lines.append(time_readable)
        for (brand, floorset), count in image_counts.items():
            full_set_count = full_set_counts[(brand, floorset)]
            line = '{:<{}} {:<{}} {:<{}} {:<{}}'.format(
                brand, max_brand_length,
                floorset, max_floorset_length,
                str(count) + ' images', max_count_length,
                str(full_set_count) + ' full sets', max_full_set_count_length
            )
            print(line)  # Print to console
            message_lines.append(line)  # Append to message for Slack

        # Join lines and send to Slack
        message_to_slack = '\n'.join(message_lines)
        # send_to_slack(message_to_slack)
        slack_client.chat_postMessage(channel=channel_id, text=message_to_slack)
        ...
        logging.error(f"Error in main execution. Details: {e}")
        raise
