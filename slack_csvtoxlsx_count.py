...
def upload_file_to_slack(file_path, channel_id):
    try:
        ...
        assert response["ok"]
    except slack_sdk.WebApiError as e:
        print(f"Error: {e.response['error']}")


def convert_csv_to_excel(csv_dir, excel_dir, channel_id):
    ...
    for csv_file in csv_files:
        ...
        # Upload the Excel file to Slack
        upload_file_to_slack(excel_path, channel_id)
...
def update_floorset_and_count(csv_path, floorset):
    ...
    if added_articles:
        ...
        print(log_msg)
        logging.info(log_msg)
        message = f"Added {len(added_articles)} new unique articles to {floorset}. Total unique articles: {total_articles}."
        ...
        if 'SOM' in floorset:
            ...
            print(log_msg)
            logging.info(log_msg)
            message_debut_flow = f"Added {debut_count} new unique debut articles and {flow_count} new unique flow articles to {floorset}."
            client.chat_postMessage(channel=channel_id, text=message_debut_flow)
        ...
        print(log_msg)
        logging.info(log_msg)
        message_no_article = f"No new unique articles added to {floorset}."
        client.chat_postMessage(channel=channel_id, text=message_no_article)
...
def main():
    logging.basicConfig(level=logging.INFO,
                        ...
                        filename='slack_csvtoxlsx_count.log',  # Name of the log file
                        filemode='a')  # Append mode, so log messages don't overwrite previous

    if check_lock():
        log_msg = "Script is currently in use by someone else."
        print(log_msg)
        logging.info(log_msg)
        return
    ...
    try:
        ...
        # channel_id = "C05Q2UQJU1G"  
        convert_csv_to_excel(csv_dir, xlsx_dir, channel_id)
        
        # Get the list of CSV files in the CSV directory
        csv_files = [file for file in os.listdir(csv_dir) if file.endswith(".csv")]
        for csv_file in csv_files:
            floorset = remove_unwanted_substrings(NON_WORD_PATTERN.sub('', DATE_PATTERN.sub('', os.path.splitext(csv_file)[0]).rstrip('_-')))
            log_msg = f"floorset read from {floorset}"
            print(log_msg)
            logging.info(log_msg)
            update_floorset_and_count(os.path.join(csv_dir, csv_file), floorset)
